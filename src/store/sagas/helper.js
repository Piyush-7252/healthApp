...
const createSaga = ({setCreateLoading, setCreateData, setCreateError}) =>
  function* ({id, url, params, cacheResponse, responseModifier}) {
    try {
      ...
      if (!cacheResponse) {
        ...
        yield put(setCreateData(id, response));
      }
    } catch (e) {
...
const customTakeLatest = (pattern, saga, ...args) =>
  fork(function* () {
    const lastTask = {};
    while (true) {
      const action = yield take(pattern);
      const id =
        action?.params?.takeLatestId ||
        action.listId ||
        action.id ||
        action.url;
      if (lastTask[id]) {
